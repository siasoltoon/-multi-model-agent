...
def test_stale_laptop_task_is_requeued_and_lease_released():
    ...
    task = Task(prompt="test", status=TaskStatus.RUNNING, worker_id="laptop-01", metadata={"worker_kind": "laptop", "lease_id": "lease-1"})
...
def test_healthy_laptop_task_is_not_recovered():
    ...
    task = Task(prompt="test", status=TaskStatus.RUNNING, worker_id="laptop-01", metadata={"worker_kind": "laptop", "worker_heartbeat_at": datetime.now(timezone.utc).timestamp()})
    store = FakeStore([task])
...
def test_non_laptop_task_is_not_recovered():
    registry = WorkerRegistry()
    task = Task(prompt="test", status=TaskStatus.RUNNING, worker_id="github-actions", metadata={"worker_kind": "github-actions"})
    store = FakeStore([task])
